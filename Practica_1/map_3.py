...
def find_path(map, start, f_cells, size_cell_x, size_cell_y):
    ...
    while len(f_cells) != 0:
        ...
        if len(neighbors) != 0:
            ...
            if len(return_points) != 0:
                ...
                for pnt in pth_return_point:
                    fill_cell(map, pnt[0], pnt[1], size_cell_x, size_cell_y, 150)
                    cv2.imshow('MAP', map)
                    cv2.waitKey(200)
        ...
        cv2.imshow('MAP', map)
        cv2.waitKey(200) # 80 buen tiempo
...
cop = free_cells.copy()
visited_cells, dirs1 = find_path(map_erosion, start_cell, cop, cell_x, cell_y)
...
# print(visited_cells[-1], dirs1[-1])
# print(get_local_goal(dirs1, visited_cells, 482, 12))
